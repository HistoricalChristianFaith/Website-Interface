...
// kjv.sqlite stores these deuterocanonical units under their own book names.
// Map canonical (formattedBook|chapter) -> (kjv_book, kjv_chapter) for KJV text lookups only.
$kjv_book_overrides = [
    'daniel|13' => ['susanna', 1],            // Susanna
    'daniel|14' => ['belandthedragon', 1],    // Bel and the Dragon
    'baruch|6'  => ['epistleofjeremiah', 6],  // Epistle of Jeremiah (already stored at ch.6)
];
...
$book_names_path = __DIR__ . '/book_names.json';
if (file_exists($book_names_path)) {
    $book_names_data = json_decode(file_get_contents($book_names_path), true);
    if ($book_names_data && isset($book_names_data['canonical_names'])) {
        foreach ($book_names_data['canonical_names'] as $canonical_name) {
            if (!isset($lookup_chaptertotals[$canonical_name])) {
                error_log("book_names.json: \"$canonical_name\" is not defined in the book arrays");
            }
        }
    }
}
